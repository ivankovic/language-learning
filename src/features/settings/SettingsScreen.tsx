...
export function SettingsScreen() {
  ...
  return (
    <Screen title="Settings">
      <button onClick={() => navigate(-1)} className="mb-6 text-sm text-slate-400">
        ← Back
      </button>

      <section className="mb-8">
        <h2 className="mb-2 text-sm font-medium text-slate-400">Languages</h2>
      ...
      <section className="mb-8">
        <h2 className="mb-2 text-sm font-medium text-slate-400">Journal prompt frequency</h2>
          ...
          {INTERVAL_OPTIONS.map((opt) => (
            <button
              ...
              className={`w-full rounded-xl px-4 py-3 text-left ${
                profile.journalIntervalDays === opt.value ? "bg-sky-500/10 ring-1 ring-sky-500 text-sky-300" : "bg-slate-900"
              }`}
            >
              {opt.label}
            </button>
      ...
      <section className="mb-8">
        <h2 className="mb-2 text-sm font-medium text-slate-400">On-device AI (Journal feedback)</h2>
        <p className="text-slate-300">{aiStatus}</p>
      </section>

      <section>
        <h2 className="mb-2 text-sm font-medium text-slate-400">Backup & restore</h2>
        ...
        />
        {importStatus && <p className="mt-2 text-xs text-slate-400">{importStatus}</p>}
      </section>
    </Screen>
  );
}
